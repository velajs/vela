...
**Behavior change:** registrations are keyed by their structural options instead of the auth instance or factory reference, so a second auth configuration with the same base path is reported by the module loader instead of becoming another instance. The process-wide reference table is removed.
