...
A Hono `HTTPException` follows the same range. The raw Hono `onError` edge skips reporting only one with a 4xx status, a deliberate client response. In 1.30.0 it skipped reporting every `HTTPException` below 500 and sent that exception's own response, so a raw middleware's `new HTTPException(302)` answered 302 with no report recorded.
