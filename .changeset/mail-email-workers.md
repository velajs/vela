...
**Behavior change:** an unclaimed `@OnInboundEmail` handler failure is reported on the core `'email'` edge (`ErrorReportContext.edge`) instead of `'queue'`; `kind` stays `'mail:inbound'`. An `ExceptionHandler` that filters inbound mail failures by `edge === 'queue'` must match `'email'`.
