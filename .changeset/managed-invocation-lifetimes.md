---
"@velajs/vela": minor
---

Add explicit managed execution scopes with injectable deferred-work lifetimes,
observable completion errors, stream-boundary coordination, and owner-qualified
asynchronous entrypoint resolution. Keep HTTP request identity separate from
non-HTTP work, and hide typed request-key storage with JavaScript private state.
