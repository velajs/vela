...
- GraphQL maps a field error's status from the shared renderer, so branded `VelaError`s and exception-owned responses reach the same public codes (`FORBIDDEN`, `NOT_FOUND`, …) as `HttpException`s. An RPC failure frame carries only `{ code, message, status }`: the rendered body's `error.code` and `error.message` when it has them (the canonical body, or an exception-owned 4xx body with that member, such as the CRUD envelope), else the status's code and a generic message; a 5xx owned body is redacted first.
