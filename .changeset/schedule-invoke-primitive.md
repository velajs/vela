...
**Behavior change:** importing `ScheduleModule.forRoot({ dispatch })` with two different dispatch kinds in one application fails bootstrap instead of leaving the policy a job uses ambiguous. Import it once, in the root module.
