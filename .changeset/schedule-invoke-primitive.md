...
**Behavior change:** a handler decorated with `@Cron` or `@Interval` that declares another required parameter, or whose first parameter does not accept its invocation, no longer compiles. Remove the native `(controller, env, ctx)` parameters and annotate the job's parameter as `CronInvocation` or `IntervalInvocation`.
...
**Behavior change:** importing `ScheduleModule.forRoot({ dispatch })` with two different dispatch policies in one application fails bootstrap instead of leaving the policy a job uses ambiguous or silently keeping the first: a different kind, or a signed policy with a different `target`, `method` or `ttlSeconds`, conflicts. Repeating one policy still deduplicates. Import it once, in the root module.
