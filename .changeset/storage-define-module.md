...
**Behavior change:** each bucket name is one module instance, keyed by the name (never by a secret). A second registration of a name with different options is reported by the module loader instead of becoming another instance, and `key` is the standard explicit instance key rather than a namespace combined with the driver identity. The process-wide identity tables, including the one that retained secret strings, are removed.
