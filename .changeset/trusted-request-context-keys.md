...
**Behavior change:** `ThrottlerGuard` publishes its decision (`{ limit, remaining?, reset }`) under the `RATE_LIMIT` request-context key from `@velajs/vela/throttler` instead of the untyped `rateLimit` Hono variable. Replace `c.get('rateLimit')` with `requestContext.get(RATE_LIMIT)`.
