...
Set `APP_LABEL` in `wrangler.toml`. Both aliases share one cache for this operation. This public greeting endpoint has no authentication; use the HTTP auth/tenant pipeline and field permission guards for protected data. See the [package guide](../../packages/graphql/README.md).
