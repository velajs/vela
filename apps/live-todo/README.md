...
- **Shared query contract**: `defineLiveQuery({ name: 'todos.list', args, result })` declares the name once; `@LiveQuery(todoListDefinition, { tags: ['todos'] })` and `createLiveClient({ queries: [todoListDefinition], ... })` consume the same definition. Tag invalidations rerun the query, and server/client validate its results.
- **Commit headers**: `@LiveInvalidates(['todos'])` invalidates after each successful mutation and stamps `Vela-Commit-Cursor`/`Vela-Commit-Epoch` on its response, with no `@Res()`; the delete handler derives its tags from the result, so a missing todo invalidates nothing. The client's optimistic layers drop exactly when a live frame's cursor passes the stamp.
...
2. Don't stamp commit headers via `ambientContainer`/`contextStorage()` on Workers — awaiting a Durable Object RPC inside hono's ALS middleware hangs the response under workerd. Stamp through `@LiveInvalidates`, which writes the response's headers directly (the `@velajs/crud` bridge does the same for you).
