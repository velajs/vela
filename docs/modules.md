...
`defineModule` builds configurable modules. Method names describe ownership:

| API | Modules |
| --- | --- |
| `forRoot` / `forRootAsync`: shared application facility | Config, Errors, Cache, Logging, Security, Throttler, I18n, Schedule, Queue, WebSocket, Live, OpenApi, BetterAuth, Authz, Cedar, CloudflareAccess, Tenant, FeatureFlags, Crud, Graphql, Rpc, Studio |
| `register` / `registerAsync`: independent local service | Http, Mail, Crypto |
| `register` / `registerAsync`: named resource | Storage, RpcClient |
| Ordinary import | EventEmitter, Health, ScheduleNode |
| Ordinary registry plus `forFeature` | Seeder |

Config and CRUD retain their feature APIs. Queue uses `forFeature([{ name,
binding }])`; i18n uses `forFeature({ en: { greeting: 'Hello' } })` and Seeder
uses `forFeature([ExampleSeeder])`. Queue's naming is Vela's convention; Nest's
queue integration calls its feature method `registerQueue`.

Only graph declarations belong in structural options. Runtime options resolve
through DI. Use `defineProvider` to check providers against their tokens and
infer factory dependencies:
...
const { ConfigurableModuleClass } = defineModule<StorageOptions, 'name', { isGlobal?: boolean }, 'register'>({
  name: 'Storage',
  methodName: 'register',
...
Consumers import `StorageModule.register({ name, driver })`, or
`StorageModule.registerAsync({ name, inject: [ENV], useFactory: (env) => ({ driver }) })`.
...
- `defaults` gives structural options their values when a call site leaves
  them out: `defaults: { path: '/files' }` makes `forRoot({})` and
  `forRoot({ path: '/files' })` one configuration with one key, and
  `setup` receives `path: '/files'` for both. The options token still
...
it means nothing else on any first-party module. Applications install exported
guards and interceptors with `APP_GUARD` / `APP_INTERCEPTOR` aliases or route
decorators. Importing an integration does not attach its guard.
...
- `ConfigurableModuleBuilder` is the NestJS-shaped facade over the same
  ...
  `setClassMethodName('forRoot')`), has no structural fields, and defaults to
  `identity: 'registration'`: every call creates an independent instance. Renaming
  its methods does not change identity. Pass `identity: 'structural'` to opt in
  to structural deduplication.
...
`DynamicModule.key` decides identity: the same class and key share an instance
within one application. Distinct classes with identical display names remain
independent. `defineModule` defaults to `identity: 'structural'`, deriving a key
from structural options and defaults. `identity: 'registration'` instead gives
each call an opaque key. `ConfigurableModuleBuilder` defaults to registration
identity. Method naming never changes identity.

HTTP, Mail and Crypto registrations are independent without caller-provided keys:

```ts
const catalog = HttpModule.register({ baseURL: 'https://catalog.example.com' });
const billing = HttpModule.register({ baseURL: 'https://billing.example.com' });
// Reuse `catalog` in several imports to share that configured client.
```

Reusing a returned definition shares its providers once within an application.
Two applications using that same definition still construct their own providers.
Factories are the reusable path for mutable runtime dependencies.

An explicit `key` overrides the generated key and must be a non-empty string
without surrounding whitespace. Repeated explicit keys retain configuration
conflict checks: plain values compare structurally, functions and instances by
reference. A reused key with different credentials, drivers, factories or other
options fails bootstrap. Storage and RPC clients additionally enforce resource
name uniqueness; assigning another key does not create a second owner of one name.

A spec's `key: options => ...` overrides the default identity strategy.
`referenceKey(...values)` compares objects and functions by reference, holding
references weakly. Never place secrets in keys: module ids appear in diagnostics.
Structural defaults and omitted or `undefined` fields compare consistently.

An ordinary `@Module` class deduplicates by class. EventEmitter, Health and
ScheduleNode have no configuration methods. Generated classes without their own
`@Module` declaration require a configured import. Mixing a bare import with a
configured definition under its default key fails when it adds configuration.
Lazy defaults from the spec and class count when comparing repeated definitions.
A conflicting global visibility flag is reported through the diagnostics policy.

The loader and OpenAPI metadata walker use the same class/key distinction.
An HTTP controller class can be mounted by only one module owner; use distinct
classes for separate HTTP routes. Middleware configured through `configure()`
retains its owner too.
...
export function contributeMessages(messages: string[]) {
...
`defineModule` for the global driver plus `forFeature` dynamic modules that provide per-queue clients, native transport contributions, and
...
- [ ] Configurable module built on `defineModule`, with methods and identity matching its ownership (a
