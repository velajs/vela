...
Standard Schema or a DTO descriptor. The first preserves a
synchronous result when the validator is synchronous; the second always returns
a promise. Both use the Standard Schema validation protocol. The async helper uses Zod’s public `safeParseAsync` when available to
...
DTO descriptors retain the concrete underlying schema. `dto.parse(value)` may
return a value or promise; use `dto.parseAsync(value)` for an always-async boundary.
...
`issues` property. Map validation failures to client errors only at input boundaries;
