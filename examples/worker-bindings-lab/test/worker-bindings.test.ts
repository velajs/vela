...
async function fetchJson(
  ...
  const response = await fetch(new Request(`https://worker-bindings-lab.test${path}`, init), env, ctx);
  return {
...
describe('Worker bindings lab consumer project', () => {
  it('covers fetch routes using every Cloudflare binding module and service', async () => {
  ...
  it('exports a Worker-shaped default object', async () => {
    ...
    await worker.queue(
      { queue: 'JOB_QUEUE', messages: [{ body: { id: 'worker-default' } }] } as MessageBatch<unknown>,
      env,
      ctx,
    );
