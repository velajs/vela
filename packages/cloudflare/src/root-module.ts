import { Module, type DynamicModule, type Type, type VelaEnv } from '@velajs/vela';

/** A static module, or a module graph built from this Worker's native environment. */
export type CloudflareRoot =
  | Type
  | DynamicModule
  | { create(env: VelaEnv): Type | DynamicModule | Promise<Type | DynamicModule> };
...
async function resolveRoot(root: Exclude<CloudflareRoot, Type>, env: VelaEnv): Promise<Type> {
 ...
 * it again.
 */
export function resolveCloudflareRoot(root: CloudflareRoot, env: VelaEnv): Promise<Type> {
 ...
 */
export async function bootstrapCloudflareRoot<R>(
  root: CloudflareRoot,
  env: VelaEnv,
