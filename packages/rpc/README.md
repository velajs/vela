...
The adapter is opt-in and requires an explicit exposure policy. `authorize: 'public'` allows the adapter's own authorization check; application globals and class/method guards still apply. Supply `authorize: async (context) => boolean` to add an application policy. The context is the existing HTTP execution context, extended with `procedure` and `callId`. Its request, trusted identity, owning module and request container come from Vela's HTTP boundary. Identity is never accepted from an RPC envelope.
...
The server uses Vela's existing exception reporter, error catalog and exception filters. A terminal filter/renderer response remains an RPC failure; its error status is retained, while arbitrary HTTP bodies are replaced by the RPC envelope. An attempted success status from an error filter becomes 500. Unknown internal errors are redacted and reported once. Only the safe code, message and status are sent; stacks, request inputs and internal error details are excluded.
