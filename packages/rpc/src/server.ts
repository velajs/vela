...
export function rpcAdapter(options: RpcAdapterOptions): RuntimeAdapter {
  ...
  return {
    ...
    onRoutesBuilt(context) {
      ...
      const registry = new RpcRegistry(context, authorize);
...
class RpcRegistry {
  ...
  readonly #app: AdapterContext;
  readonly #authorize: RpcAdapterOptions['authorize'];
  constructor(context: AdapterContext, authorize: RpcAdapterOptions['authorize']) {
    this.#app = context;
  ...
  async handle(c: VelaContext): Promise<Response> {
    ...
    try {
      const globals = this.#app.routeManager.getGlobalComponents();
      filters = [
        ...(
