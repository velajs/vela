...
- Feature subpaths: `@velajs/vela/cache`, `/throttler`, `/schedule`, `/events`, `/health`, `/security` (SecurityModule, signed-URL primitives, nonce store), `/logging`, `/openapi` (`OpenApiModule`, `@ApiResponse`, OpenAPI documents), `/contract` (browser-safe `defineRoute` contracts and `ContractApp` client types), `/dispatch` (signed internal dispatch), `/http-client`, `/validation`, `/websocket`, `/queue`, `/live`, `/i18n`, `/seeder`, `/streaming`, `/observability`, `/schedule-node` and `/websocket-node` (Node/Bun only).
...
4. **Schemas carry runtime evidence.** Declare the route's contract on the method decorator: `@Post({ response: Todo })` validates/strips the result, documents it, types the generated client and fails to compile when the handler's return type does not match. Pass request schemas (or `defineDto(schema)` descriptors from `@velajs/vela/validation`) to parameter decorators: `@Body(schema)`, `@Query('page', schema)`, `@Param('id', schema)` validate with `ValidationPipe` (400 on invalid input) and feed OpenAPI; DTO descriptors are not classes. To share a contract with a browser, use `defineRoute` from `@velajs/vela/contract` and `@Post(contract)`. POST answers 201, `response: null` 204, other methods 200. Forms/multipart opt in per route (`body: { multipart: limits }`); routes are JSON-only otherwise. `zod` is an application dependency; core uses Standard Schema.

5. **Use the module API for its ownership.** Shared facilities expose `forRoot`/`forRootAsync`; Http, Mail and Crypto expose independent `register`/`registerAsync`; Storage and RpcClient use named registrations. Queue, I18n and Seeder contribute with `forFeature`. EventEmitter, Health and ScheduleNode are ordinary imports. `defineModule` defaults to structural identity; `ConfigurableModuleBuilder` defaults to opaque registration identity. Reuse one returned definition to share within an application. Explicit keys retain conflict checks. Applications attach exported guards/interceptors with `APP_*` aliases or route decorators; imports alone do not install them. Structural fields shape graph declarations; async factories return runtime options.
...
| `vela add d1\|kv\|r2\|queue <BINDING>` | Create the resource with the project's Wrangler, refresh `wrangler types`, register a `BindingsModule` token or `QueueModule.forFeature([])` |
...
- `VelaFactory.createApplicationContext(rootModule, { env?, diagnostics?, configureContainer? })` is Nest's `createApplicationContext`: the module graph, providers, lifecycle hooks and `entrypoints` with no HTTP routes, initialized before it resolves. `VelaApplicationContext` has `get(token, { strict? })` (app-wide; `strict` resolves as the selected module sees it), `resolve(token, scope?, { strict? })` (async; transient anew, request-scoped only in the scope passed), `select(Module | DynamicModule)`, idempotent `init()`, `close()`, `dispose()`. `VelaApplication` extends it; every Cloudflare Durable Object boots one.
...
For Cloudflare export `createCloudflareWorker(AppModule)`, or define the app once with `const app = defineCloudflareApp(AppModule, options)` and `export default app.worker` when the entry also exports classes built from it (`VelaDurableObject(app, Host)`, `VelaWebSocketDurableObject(app)` from `@velajs/cloudflare/durable-objects`, `VelaWorkflow(app, Host)` from `/workflows`, `VelaEntrypoint(app, Host, { rpc })` from `/entrypoints`; `@OnEmail()`/`@OnTail()` from `/email` and `/tail` add the Worker's `email`/`tail` handlers); it seeds the native environment as the framework `ENV` before bootstrap and isolates applications by environment. Its adapter also wires the core `WebSocketModule` and `LiveModule` to Durable Objects (`WS_TRANSPORT`/`LIVE_PLATFORM`), so the same `AppModule` runs on Node and Workers. Inject bindings with `@InjectEnv()` or `inject: [ENV]`, typed by `wrangler types` (`worker-configuration.d.ts`); never hand-write an environment `InjectionToken`. Read `references/cloudflare.md`. New projects: read `assets/project-scaffold.md`.
...
`ModuleOptions`: `imports`, `controllers`, `providers`, `exports`, `lazy?`. Make a module global with `@Global()` (or one instance with the `isGlobal` extra of `forRoot`); only **exported** tokens go app-wide. Provider kinds: `useValue`, `useClass`, `useFactory` + `inject`, `useExisting`. Shared facilities use `forRoot`/`forRootAsync`; local or named services use `register`/`registerAsync`; declaration contributions use `forFeature`. See `references/modules-and-di.md` and `references/lazy-and-lifecycle.md`.
...
| `references/validation.md` | Route options (`response`, `status`, `format`, `body`), `defineRoute` contracts (`@velajs/vela/contract`), forms/multipart limits, array-aware `@Query`, `defineDto`/`ValidationPipe` (`@velajs/vela/validation`) |
| `references/serialization.md` | `response` schemas, async output schemas, `defineSerializer`, explicit domain projections and private state |
...
| `references/queues.md` | `@velajs/vela/queue`: `QueueModule.forRoot`/`forFeature`, `@InjectQueue`/`QueueClient` (`add`, `addBulk`), `@Processor`/`@Process`, inline driver, `cloudflareQueues()`, signed dispatch, `dispatchQueueJob` |
...
| `references/cloudflare.md` | `@velajs/cloudflare`: Workers adapter, KV/D1/R2/Queues/Durable Objects, Workflows, service entrypoints, email and tail handlers, `wrangler.toml`, `nodejs_compat` |
...
**`[vela] XModule#key was imported again with different options`** → Two imports share one `(class, key)` but were built from different options (different closures, even with the same source, or distinct class instances such as tokens), so bootstrap fails in every diagnostics mode. Import one shared definition (export a const of the `DynamicModule`) instead of building it twice, or pass a distinct `key` per configuration (local HTTP registrations already receive independent keys).
...
**`Multiple providers found for 'X' ...`** (`MultipleProvidersFoundError`) → Two module instances export the same token (e.g. `HttpModule.register({ baseURL: a })` and `HttpModule.register({ baseURL: b })`). Import only one, or use a per-instance accessor.
