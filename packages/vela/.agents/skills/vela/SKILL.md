...
6. **Use checked providers and inferred tokens.** Register values/classes/factories/aliases as Nest literals in `@Module({ providers })` (`{ provide: TOKEN, useValue }`) or with `defineProvider(TOKEN, strategy)`; a factory declares its parameters' tokens in `inject`, and one without parameters may omit it. Only an array literal written in `@Module` is type-checked per element; literals in `DynamicModule.providers`, `defineModule` `setup` contributions or a shared `Provider[]` list are validated at runtime only, so use `defineProvider` there when the value type matters. Resolve with `get(TOKEN)`; use `RequestContextKey<Value>` for typed request state. Raw values remain unknown until parsed.
