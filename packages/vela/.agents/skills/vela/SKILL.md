...
6. **Use checked providers and inferred tokens.** Register values/classes/factories/aliases as Nest literals in `@Module({ providers })` (`{ provide: TOKEN, useValue }`) or with `defineProvider(TOKEN, strategy)`; a factory declares its parameters' tokens in `inject`, and one without parameters may omit it. Resolve with `get(TOKEN)`; use `RequestContextKey<Value>` for typed request state. Raw values remain unknown until parsed.
