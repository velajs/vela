...
- Feature subpaths: `@velajs/vela/cache`, `/throttler`, `/schedule`, `/events`, `/health`, `/security` (SecurityModule, CORS, signed-URL primitives, nonce store), `/logging`, `/openapi` (`OpenApiModule`, `@ApiResponse`, OpenAPI documents), `/contract` (browser-safe `defineRoute` contracts and `ContractApp` client types), `/dispatch` (signed internal dispatch), `/http-client`, `/validation`, `/websocket`, `/queue`, `/live`, `/i18n`, `/seeder`, `/storage`, `/streaming`, `/observability`, `/schedule-node` and `/websocket-node` (Node/Bun only).
