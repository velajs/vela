...
- `@OnTail()` (`@velajs/cloudflare/tail`) receives `TraceItem[]` (kind `cf:tail`); every handler runs; failures are reported (`edge: 'tail'`) and never thrown.
