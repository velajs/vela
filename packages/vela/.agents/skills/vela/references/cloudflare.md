...
For portable jobs, use `QueueModule.forRoot({ driver: cloudflareQueues() })` (from `@velajs/cloudflare/queues`) with `QueueModule.forFeature([{ name, binding }])` and `@InjectQueue(name)`: the driver reads the producer binding from `ENV` per send, `addBulk` uses chunked `sendBatch`, and batches no `@QueueConsumer` claims are routed to `@Processor`s by each job's logical queue. See `queues.md`.
...
- Storage: `StorageModule` from `@velajs/storage` is the only storage module. `StorageModule.register({ driver: r2Storage({ binding: 'FILES' }) })` with `r2Storage` from `@velajs/cloudflare/storage`; the bucket is read on the first storage operation of each application.
