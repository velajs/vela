...
`@Body()` parses JSON only unless the route opts into a form: a body must arrive as `application/json` or a `+json` media type (parameters like `charset` are fine), otherwise the request fails with 415 `unsupported_media_type`; malformed JSON is 400 and a request without a body yields `undefined`. `@Post('/upload', { body: { multipart: { maxFileBytes } } })` or `body: { form: {} }` makes the route accept only that form encoding, within its limits (413 beyond them). `@Body()` without a schema validates a parameter class carrying a static Standard Schema (a named `@Body('item')` validates that member). Send `content-type: application/json` in tests (`@velajs/testing` does this for you). Raw Hono routes can use `readJsonBody(c)` for the same rule.

`@Query()` and `@Query('tag')` return repeated keys (`?tag=a&tag=b`) as arrays; keys a query schema declares as arrays are arrays even when sent once, and other keys stay strings. Without a schema, a named parameter typed `string`, `number` or `boolean` reads the first value and one typed as an array (without a pipe) always reads an array.
