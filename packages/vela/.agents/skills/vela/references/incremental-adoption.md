...
| NestJS | Vela | Notes |
|---|---|---|
| class-validator + class-transformer DTOs | **Schemas**: route options (`@Post({ response })`), a schema passed to the parameter decorator (`@Body(dto)`, validated by `ValidationPipe`), or a DTO class carrying a static Standard Schema (`@Body() body: CreateTodo`) | core never imports class-validator — see `references/validation.md` |
...
| `app.enableCors()` / `NestFactory.create(m, { cors })` | `app.enableCors(options?)` or the `cors` create-option (`true` or `CorsOptions`); Cloudflare: `createCloudflareWorker(AppModule, { cors })` | served by Hono's `cors` before routes and guards, default methods as in Nest; there is no `CorsModule`, and it cannot be combined with `SecurityModule`'s `cors` (pass `cors: false` there) |
...
`VelaFactory.create(rootModule, options?)` is **always async** and returns `Promise<VelaApplication>`. `VelaCreateOptions` in full: `globalPrefix`, `globalPrefixOptions`, `versioning`, `middleware`, `cors`, `getClientIp`, `adapters`, `env`, `configureContainer`, `ambientContainer`, `bodyLimit`, `security`, `diagnostics` — nothing else. Authoring configurable modules uses `defineModule`, not hand-wired `forRoot`; see the module reference.
