...
Use `defineEndpoint({ input, output, status? })` plus `@Endpoint(definition)` for one runtime-validated contract; see `validation.md`. Context parameters after the endpoint input (such as `@CurrentUser()`) are not documented and never reach generated clients. A parameter parser in `@Body(new ValidationPipe(dto))` also supplies request schema metadata. `@ApiResponse` documents a result but does not validate it; TypeScript interfaces alone carry no schema.
