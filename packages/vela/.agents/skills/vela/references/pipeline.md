...
- Matching follows Hono's router exactly: each path target is registered on the Hono app as a do-nothing route, so the router that picks the serving route also decides what a target matches (a parent app's base path, decoding, `{regex}` constraints). A `forRoutes()` target is registered as its pattern, its trailing `*` form and a `:_{[^]+}` parameter form, which makes Hono fall back from its RegExpRouter to its TrieRouter beside a deeper route.
- Constraints behave as in routes: `.` does not match decoded line terminators (`%0A`, `%E2%80%A8`), so write `[\s\S]` or a trailing `*` for a catch-all, and wrap a top-level `|` in `(?:...)`. An invalid constraint, or one that matches an empty segment, throws at route build.
- Nest's trailing wildcards become Hono patterns: `cats/*path` and `cats/(.*)` become `cats/:path{[\s\S]+}` (one or more characters, never `/cats` itself); a trailing `cats/{*splat}` becomes Hono's `cats/*` (which also matches `/cats`). A Nest wildcard before the last segment (`files/*path/download`, `files/*a/*b`) and other group, optional or named-wildcard syntax (`:id(\d+)`, `users{/:id}`, `ab*cd`, a mid-path `:id?`) throw at route build; use `:id{[0-9]+}` for a constrained segment.
