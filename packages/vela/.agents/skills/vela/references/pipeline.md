...
A handler function reads the metadata of the method it was decorated as. In the list form, that metadata counts only when a listed class is, or extends, the class that declared it: a method one controller decorates never lends its metadata to a sibling controller inheriting the same method. When several controllers decorate the same function (one inherited method decorated per controller, without decorator syntax), the function cannot name its controller: reading through `context.getHandler()` throws, so pass the `ExecutionContext` there.
