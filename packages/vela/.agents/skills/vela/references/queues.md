...
- `QueueModule.forRoot({ driver?, dispatch? })` is global and imported **once** per application; two different `forRoot` configurations fail bootstrap. `driver` defaults to `inline()`. `forRootAsync({ inject, useFactory })` resolves the same options.
