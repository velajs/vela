...
- `@SubscribeMessage(event)` — handler for an inbound message event (stackable). A gateway also serves the handlers an ancestor class declares on methods it inherits unchanged (as in Nest); its own declaration of an event wins, and an override without the decorator is not a handler.
...
- `@WebSocketServer()` (or `@Inject(WS_SERVER)` in a gateway's constructor, declared or inherited) injects the gateway's own `WsServer`: its broadcasts carry the gateway path and reach only this gateway's sockets in the current isolate, even when another gateway uses the same room id; `afterInit(server)` receives the server that handle pushes through (same sockets, not the same object). `WS_SERVER` injected outside a gateway addresses every gateway's sockets. To push from anywhere else (HTTP handlers, queue consumers, crons, another gateway), inject `Gateways`.
- `WebSocketModule` connects each gateway's server before any lifecycle hook, from the one `WS_SERVER` the gateway's module sees (async `useFactory` providers included); a module that sees none gets its `WebSocketModule` instance's own, and one that sees two or more (several `WebSocketModule` instances, or another module's `WS_SERVER` beside one) fails bootstrap naming each provider's module. To test a gateway's `@WebSocketServer()` pushes against a double, keep `WebSocketModule.forRoot()` imported and provide `WS_SERVER` in the gateway's module or `overrideProvider(WS_SERVER)` in the testing module; a `WS_SERVER` without `WebSocketModule` connects nothing, and the gateway's server refuses each push. `Gateways` pushes do not go through `WS_SERVER`.
