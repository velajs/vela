...
- `WebSocketModule` connects each gateway's server before any lifecycle hook, from the one `WS_SERVER` the gateway's module sees (async `useFactory` providers included); a module that sees none, or only the servers of several `WebSocketModule` instances, gets a `WebSocketModule` instance's own, and one that sees another module's `WS_SERVER` beside those fails bootstrap naming each provider's module. To test a gateway's `@WebSocketServer()` pushes against a double, keep `WebSocketModule.forRoot()` imported and provide `WS_SERVER` in the gateway's module or `overrideProvider(WS_SERVER)` in the testing module; a `WS_SERVER` without `WebSocketModule` connects nothing, and the gateway's server refuses each push. `Gateways` pushes do not go through `WS_SERVER`.
