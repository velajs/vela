...
it('normalizes Standard issue paths and does not downgrade unexpected errors carrying issues', async () => {
  ...
  try {
    ...
    expect(error instanceof BadRequestException && error.getDetails()).toMatchObject({
      issues: [{ path: ['users', 0, 'email'], message: 'Required' }],
    });
  }
...
it('maps async Standard input failures without forwarding vendor values', async () => {
  ...
  try {
    ...
    expect(details).toMatchObject({ issues: [{ message: 'Invalid', path: ['value'] }] });
