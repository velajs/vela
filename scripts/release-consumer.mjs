...
const artifactDir = resolve(process.argv[2] ?? ".modernization/release-artifacts");
...
const workspaceVersions = Object.fromEntries(
  JSON.parse(execFileSync("pnpm", ["list", "-r", "--depth", "-1", "--json"], {
...
await writeFile(
  ...
  JSON.stringify({ path: consumer, status: "passed", manifestIntegrity: `sha512-${createHash('sha512').update(await readFile(join(artifactDir, 'manifest.json'))).digest('base64')}` }, null, 2) + "\n",
);
console.log(
