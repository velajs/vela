...
const owner = (ctx: { identity: { userId?: string } }, r: { authorId: string }) => ctx.identity.userId === r.authorId;
...
describe('composition', () => {
  it('anyOf is OR', async () => {
    ...
    expect(await p({ identity: { userId: 'x', roles: ['editor'] } }, { authorId: 'u1' })).toBe(true); // perm
  ...
  it('allOf is AND', async () => {
    ...
    expect(await p({ identity: { userId: 'u1', roles: ['editor'] } }, { authorId: 'u1' })).toBe(true);
  ...
  it('FAIL-CLOSED: a throwing policy denies its branch (never allows)', async () => {
    const boom = () => { throw new Error('policy bug'); };
...
describe('mask (fail-closed)', () => {
  it('returns the transform result', () => {
    expect(mask((_c: unknown, r: { ssn: string }) => r.ssn.slice(-4))({}, { ssn: '123456789' })).toBe('6789');
  });
  it('redacts to null when the transform throws', () => {
    expect(mask(() => { throw new Error('bad'); })({}, {})).toBeNull();
  });
});
